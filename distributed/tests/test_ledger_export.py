...
def test_oeis_ready_data_line_proven_only_and_under_260(fam):
    units = {"lehmer-q2-00000000-00005000": {"state": "verified", "n_hi": 5000, "results": [],
             ...
             "positive_claims": [{"n": 1019, "variant": "m1", "discoverer_login": "alice", "verifier_login": "bob", "maintainer_pari": "isprime", "ci_confirmed": True},
                                 {"n": 1039, "variant": "m1", "discoverer_login": "alice", "verifier_login": "", "maintainer_pari": "none", "ci_confirmed": True}]}}
    ledger = {"units": units, "verified_through": 1030, "contributors": {}}
    ready = oeis_ready(ledger, fam, {})
    a3 = ready["sequences"]["A3"]
    assert a3["data"] == "4,5,1019" and len(a3["data"]) <= 260
    assert [t["n"] for t in a3["terms"]] == [4, 5, 1019] and a3["pending_terms"] == [{"n": 1039, "variant": "m1", "reason": "position_undetermined"}]
